...
/*
 * This file is a bit confusing. I know that there is just reading and writing of data using the buffers.
 * I can see the advancing of the pointers, the copying of bytes based on size from buffer to dest var.
 * TODO: Look into pointer manipulation in C.
 */

// Reading data
uint8_t unpack_u8(const uint8_t **buf)
{
    uint8_t val = **buf; // get the value of the byte from the pointer, double dereferencing.
    (*buf)++;            // advance the pointer by one byte
